...
def init_or_get_youtube_client(tool_context: ToolContext):
    """
    Initializes the YouTube Data API client using the API key from session state or configuration.
    
    Returns:
        tuple: (youtube_client, error_message)
    """
    # 1. Try to get key from session state
    api_key = tool_context.state.get("youtube_api_key")

    # 2. Fallback to configuration (environment variable)
    if not api_key:
    ...
    if not api_key:
        error_msg = (
            ...
            "Please ensure YOUTUBE_API_KEY is set in your .env file or provide one using the 'store_youtube_api_key' tool."
        )
...
def store_youtube_api_key(api_key: str, tool_context: ToolContext) -> str:
    """
    Stores a YouTube API key in the current session state.
    Use this when the user provides a key manually after a SYSTEM REJECTION.
    ...
    """
    tool_context.state["youtube_api_key"] = api_key.strip()
    return "YouTube API Key has been stored successfully for this session. You can now proceed with YouTube-related tasks."
